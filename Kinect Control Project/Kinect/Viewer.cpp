...
float _skeletonData[16 * 4]; // hand left (x,y,z,confidence), hand right (x,y,z,confidence), elbow ..., shoulder..., hip..., torso (x,y,z,confidence), head (x,y,z,confidence)
...
// ADDED FOLLOWING FUNCTION:
void prepareSkeletalData(const nite::UserData& player)
{
	const nite::SkeletonJoint& head = player.getSkeleton().getJoint(nite::JOINT_HEAD);
	const nite::SkeletonJoint& torso = player.getSkeleton().getJoint(nite::JOINT_TORSO);
	const nite::SkeletonJoint& leftShoulder = player.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER);
	const nite::SkeletonJoint& leftElbow = player.getSkeleton().getJoint(nite::JOINT_LEFT_ELBOW);
	const nite::SkeletonJoint& leftHand = player.getSkeleton().getJoint(nite::JOINT_LEFT_HAND);
	const nite::SkeletonJoint& rightShoulder = player.getSkeleton().getJoint(nite::JOINT_RIGHT_SHOULDER);
	const nite::SkeletonJoint& rightElbow = player.getSkeleton().getJoint(nite::JOINT_RIGHT_ELBOW);
	const nite::SkeletonJoint& rightHand = player.getSkeleton().getJoint(nite::JOINT_RIGHT_HAND);
	const nite::SkeletonJoint& leftHip = player.getSkeleton().getJoint(nite::JOINT_LEFT_HIP);
	const nite::SkeletonJoint& rightHip = player.getSkeleton().getJoint(nite::JOINT_RIGHT_HIP);

	EnterCriticalSection(&_dataCriticalSection);
	_skeletonData[0] = leftHand.getPosition().x;
	_skeletonData[1] = leftHand.getPosition().y;
	_skeletonData[2] = leftHand.getPosition().z;
	_skeletonData[3] = leftHand.getPositionConfidence();
	_skeletonData[4] = rightHand.getPosition().x;
	_skeletonData[5] = rightHand.getPosition().y;
	_skeletonData[6] = rightHand.getPosition().z;
	_skeletonData[7] = rightHand.getPositionConfidence();

	_skeletonData[8] = leftElbow.getPosition().x;
	_skeletonData[9] = leftElbow.getPosition().y;
	_skeletonData[10] = leftElbow.getPosition().z;
	_skeletonData[11] = leftElbow.getPositionConfidence();
	_skeletonData[12] = rightElbow.getPosition().x;
	_skeletonData[13] = rightElbow.getPosition().y;
	_skeletonData[14] = rightElbow.getPosition().z;
	_skeletonData[15] = rightElbow.getPositionConfidence();

	_skeletonData[16] = leftShoulder.getPosition().x;
	_skeletonData[17] = leftShoulder.getPosition().y;
	_skeletonData[18] = leftShoulder.getPosition().z;
	_skeletonData[19] = leftShoulder.getPositionConfidence();
	_skeletonData[20] = rightShoulder.getPosition().x;
	_skeletonData[21] = rightShoulder.getPosition().y;
	_skeletonData[22] = rightShoulder.getPosition().z;
	_skeletonData[23] = rightShoulder.getPositionConfidence();

	_skeletonData[24] = leftHip.getPosition().x;
	_skeletonData[25] = leftHip.getPosition().y;
	_skeletonData[26] = leftHip.getPosition().z;
	_skeletonData[27] = leftHip.getPositionConfidence();
	_skeletonData[28] = rightHip.getPosition().x;
	_skeletonData[29] = rightHip.getPosition().y;
	_skeletonData[30] = rightHip.getPosition().z;
	_skeletonData[31] = rightHip.getPositionConfidence();

	/*_skeletonData[32] = leftKnee.getPosition().x;
	_skeletonData[33] = leftKnee.getPosition().y;
	_skeletonData[34] = leftKnee.getPosition().z;
	_skeletonData[35] = leftKnee.getPositionConfidence();
	_skeletonData[36] = rightKnee.getPosition().x;
	_skeletonData[37] = rightKnee.getPosition().y;
	_skeletonData[38] = rightKnee.getPosition().z;
	_skeletonData[39] = rightKnee.getPositionConfidence();

	_skeletonData[40] = leftAnkle.getPosition().x;
	_skeletonData[41] = leftAnkle.getPosition().y;
	_skeletonData[42] = leftAnkle.getPosition().z;
	_skeletonData[43] = leftAnkle.getPositionConfidence();
	_skeletonData[44] = rightAnkle.getPosition().x;
	_skeletonData[45] = rightAnkle.getPosition().y;
	_skeletonData[46] = rightAnkle.getPosition().z;
	_skeletonData[47] = rightAnkle.getPositionConfidence();

	_skeletonData[48] = leftFoot.getPosition().x;
	_skeletonData[49] = leftFoot.getPosition().y;
	_skeletonData[50] = leftFoot.getPosition().z;
	_skeletonData[51] = leftFoot.getPositionConfidence();
	_skeletonData[52] = rightFoot.getPosition().x;
	_skeletonData[53] = rightFoot.getPosition().y;
	_skeletonData[54] = rightFoot.getPosition().z;
	_skeletonData[55] = rightFoot.getPositionConfidence(); */

	_skeletonData[56] = torso.getPosition().x;
	_skeletonData[57] = torso.getPosition().y;
	_skeletonData[58] = torso.getPosition().z;
	_skeletonData[59] = torso.getPositionConfidence();

	_skeletonData[60] = head.getPosition().x;
	_skeletonData[61] = head.getPosition().y;
	_skeletonData[62] = head.getPosition().z;
	_skeletonData[63] = head.getPositionConfidence();
...
void SampleViewer::Display()
{
	...
	if (rc != nite::STATUS_OK)
	{
		...
		// ----Added Line: Pause to the window to stop it closing
		system("pause");
		return;
	}
